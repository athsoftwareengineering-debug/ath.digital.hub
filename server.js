...
// Update all orders - daily check for expiry and reminders
function updateAllOrdersRemainingDays() {
  ...
  orders.forEach(order => {
    if (order.status === 'approved' && order.endDate) {
      ...
      if (alertDays.includes(remaining) && order.lastAlertDay !== remaining && remaining > 0) {
        ...
        const reminderMsg = `⚠️ *REMINDER* Order #${order.id}\n\n📞 ${order.phone}\n📦 ${order.packageName}\n⏳ ${remaining} days remaining!\n📅 Expires: ${new Date(order.endDate).toLocaleDateString()}`;
        
        sendTelegramMessage(ADMIN_CHAT_ID, reminderMsg);
  ...
  console.log(`✅ Updated: ${updatedCount} | Expired: ${expiredCount}`);
...
app.post('/api/admin/update-order', async (req, res) => {
  ...
  if (status === 'approved' && !order.startDate) {
    ...
    await sendTelegramMessage(ADMIN_CHAT_ID, `✅ Order #${orderId} approved! 30 days countdown started.\n📞 ${order.phone}\n📅 Expires: ${endDate.toLocaleDateString()}`);
    ...
    order.status = status;
    await sendTelegramMessage(ADMIN_CHAT_ID, `❌ Order #${orderId} ${status === 'rejected' ? 'rejected' : 'updated'}.`);
  }
...
app.listen(PORT, async () => {
  ...
  console.log(`=================================`);
  console.log(`📱 Website: https://ath-digital-hub.onrender.com`);
