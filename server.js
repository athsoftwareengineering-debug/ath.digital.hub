...
function getStatusMessage() {
    ...
    if (salesHours.mode === 'manual') {
        return isOpen ? '🟢 ဆိုင်ဖွင့်ချိန် (Manual Mode)' : '🔴 ဆိုင်ပိတ်ချိန် (Manual Mode)';
    }
    return isOpen ? `🟢 ဆိုင်ဖွင့်ချိန် (${salesHours.startHour}:00 - ${salesHours.endHour}:00)` : `🔴 ဆိုင်ပိတ်ချိန်။ နံနက် ${salesHours.startHour}:00 မှ ညနေ ${salesHours.endHour}:00 အတွင်းမှသာ ဝယ်ယူနိုင်ပါသည်။`;
}
...
// ==================== RATE LIMITING ====================
const apiLimiter = rateLimit({ windowMs: 15 * 60 * 1000, max: 500, message: { error: 'Too many requests' }, keyGenerator: (req) => req.ip });
const chatLimiter = rateLimit({ windowMs: 60 * 1000, max: 60, message: { error: 'Too many chat requests' }, keyGenerator: (req) => req.ip });
const adminLimiter = rateLimit({ windowMs: 15 * 60 * 1000, max: 300, message: { error: 'Too many admin requests' }, keyGenerator: (req) => req.ip });
...
app.post('/api/chat/send', chatLimiter, async (req, res) => {
    try {
        ...
        if (!user_id || !message || message.trim() === '') return res.status(400).json({ success: false, error: 'Invalid message' });
        const { data, error } = await supabase.from('chat_messages').insert([{ user_id: user_id, username: username || 'User', message: message.substring(0, 500), is_admin: false, created_at: new Date().toISOString() }]).select();
        if (error) throw error;
...
app.post('/api/chat/admin/send', isAuthenticated, chatLimiter, async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('chat_messages').insert([{ user_id: user_id || 'admin', username: username || 'Admin', message: message.substring(0, 500), is_admin: true, created_at: new Date().toISOString() }]).select();
        if (error) throw error;
        res.json({ success: true, message: data[0] });
    } catch (error) { res.status(500).json({ success: false, error: error.message }); }
});
...
app.delete('/api/chat/messages/:id', isAuthenticated, async (req, res) => {
    try { const { id } = req.params; const { error } = await supabase.from('chat_messages').delete().eq('id', id); if (error) throw error; res.json({ success: true }); } catch (error) { res.status(500).json({ success: false, error: error.message }); }
});

// ==================== PRIVATE 1-ON-1 CHAT API (WORKING) ====================
...
app.get('/api/chat/private/:userId', chatLimiter, async (req, res) => {
    try {
        ...
        if (currentUserId === 'admin') {
            await supabase.from('private_chat_messages').update({ is_read: true }).eq('sender_id', userId).eq('receiver_id', 'admin');
        }
        res.json({ success: true, messages: data || [] });
    } catch (error) { res.status(500).json({ success: false, error: error.message }); }
});
